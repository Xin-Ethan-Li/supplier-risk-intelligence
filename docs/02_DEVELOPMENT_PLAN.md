...
- [ ] 定义供应商事件和特征数据字典。
- [ ] 编写固定随机种子的合成数据生成器。
- [ ] 生成带时间字段、噪声和约 2% 高风险标签的数据集。
- [ ] 实现时间切分训练和测试。
- [ ] 在训练 Fold 内处理类别不平衡。
- [ ] 训练 XGBoost 并选择业务阈值。
- [ ] 计算 Recall、Precision、PR-AUC 和 Confusion Matrix。
- [ ] 实现 SHAP 或兼容的特征贡献输出。
- [ ] 保存模型、特征 Schema、版本和评估 Metadata。
- [ ] 增加训练数据和模型封装测试。
...
- [ ] 编写顶级 README。
- [ ] 加入架构图、截图和 Demo GIF。
- [ ] 编写 Quick Start 和 Troubleshooting。
- [ ] 编写 Evaluation、Security 和 Limitations 文档。
- [ ] 创建公开 GitHub 仓库。
- [ ] 配置分支保护和 CI 状态检查。
